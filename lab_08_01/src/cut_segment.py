...
def update_range(w_scalar: float, d_scalar: float, t_range: list[float]) -> bool:  
    if d_scalar == 0:
        return w_scalar >= 0
...
def cyrus_beck(polygon: QPolygon, segment: QLine) -> tuple[QLine, bool]:
    ...
    res_point1 = QPointF(point1.x() + d.x() * t_range[0], point1.y() + d.y() * t_range[0]).toPoint()
    res_point2 = QPointF(point1.x() + d.x() * t_range[1], point1.y() + d.y() * t_range[1]).toPoint()
