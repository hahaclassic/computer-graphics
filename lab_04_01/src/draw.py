...
class CirclePlotter(Plotter):
    ...
    def canonical(self, circle: Circle) -> list[QPoint]:
        ...
        for x in range(start, end):
            y = math.sqrt(circle.radius**2 - (x - circle.center.x())**2) + circle.center.y()
    ...
    def bresenham(self, circle: Circle) -> list[QPoint]:
        ...
        add_symmetrical_points(points, circle.center + QPointF(x, y), circle.center, True)
...
class EllipsePlotter(Plotter):
    ...
    def canonical(self, ellipse: Ellipse) -> list[QPoint]:
        ...
        limit = int(ellipse.center.x() + major / math.sqrt(1 + minor**2 / major**2))

        for x in range(int(ellipse.center.x()), limit + 1):
            y = math.sqrt(major**2 * minor**2 \
                - (x - ellipse.center.x())**2 * minor**2) / major + ellipse.center.y()
            add_symmetrical_points(points, QPointF(x, y), ellipse.center, False)
           
        limit = int(ellipse.center.y() + minor / math.sqrt(1 + major**2 / minor**2))

        for y in range(limit, int(ellipse.center.y()) - 1, -1):
            x = math.sqrt(major**2 * minor**2 \
                - (y - ellipse.center.y())**2 * major**2) / minor + ellipse.center.x()
            add_symmetrical_points(points, QPointF(x, y), ellipse.center, False)
    ...
    def bresenham(self, ellipse: Ellipse) -> list[QPoint]:
        ...
        add_symmetrical_points(points, ellipse.center + QPointF(x, y), ellipse.center, False)
    ...
    def midpoint(self, ellipse: Ellipse) -> list[QPoint]:
        ...
        delta = minor**2 * (x + 0.5)**2 + major**2 * (y - 1)**2 - major**2 * minor**2
